...
**v0.1.1 增加了六个公开真实案例和十项回归测试。** 中文流程图及四张 NASA 页面通过自动阻断检查；
密集表格仍有一处字体与网格冲突，保留失败诊断稿。真实案例、可编辑文件和像素测量见
[测试报告](docs/real_cases.md)；目前全套 43 项测试通过，不能据此声称所有图片均可完美还原。
...
- 暂无旋转文本、竖排、任意矢量路径、渐变、复杂数学排版的专用导出支持；标准连线箭头头部可能偏小。
- 自动验收以 LibreOffice/PDFium 为依据；原生 PowerPoint、WPS 兼容性需单独核验。
- 已有六个公开真实案例，仍缺用户自己的失败样本；密集表格也明确保留了一例未通过。
