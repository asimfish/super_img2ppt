...
**“减少 AI 感”不等于简化内容。** 对于素材风格混杂、文字拉伸、图标细节不自然、矩阵透视和线条不一致的图，使用额外的**语义重绘模式**：Agent 理解原图后，在保留文字、公式、数值、关系和信息密度的前提下，重新制作图标、排字和局部几何。它通过独立场景调用原有构建与检查流程，不是自动滤镜。

可以说：**“保留这张图的信息和丰富程度，重新绘制不自然的元素，统一制图方式，并给我可编辑版本。”** 忠实版与重绘版分开保留。[语义重绘流程](skills/super-img2ppt/references/redraw.md)

下面的 `restyle` 是较轻量的样式调整，仅适用于字体、配色和线条等修改：

想在转换时减少花哨渐变、统一字体和线条，可以说：**“转成可编辑 PPT，并额外统一字体、配色和边框样式。”** 默认仍然忠实转换，美化是独立选项。
